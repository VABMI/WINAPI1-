...
namespace iphelper
{
    ...
    [DllImport("iphlpapi.dll",SetLastError=true)]
    private static extern 
        UInt32 GetIpStatistics(
        ref MIB_IPSTATS pmips
        );

    public static MIB_IPSTATS GetIpStatistics()
    {
        UInt32 error_code=0;
        return iphelper.GetIpStatistics(true,ref error_code);
    }
    public static MIB_IPSTATS GetIpStatistics(bool b_verbose,ref UInt32 error_code)
    {
        MIB_IPSTATS mips=new MIB_IPSTATS();
        error_code=GetIpStatistics(ref mips);
        if ((error_code!=0) && b_verbose)// error
        {
            string str_msg=Tools.API.API_error.GetAPIErrorMessageDescription(error_code);
            System.Windows.Forms.MessageBox.Show( str_msg,"Error",
    ...
    #region GetUdpStatistics
        [DllImport("iphlpapi.dll",SetLastError=true)]
        private static extern 
            UInt32 GetUdpStatistics(
            ref MIB_UDPSTATS pStats
            );
        public static MIB_UDPSTATS GetUdpStatistics()
        {
            UInt32 error_code=0;
            return iphelper.GetUdpStatistics(true,ref error_code);
        }
        public static MIB_UDPSTATS GetUdpStatistics(bool b_verbose,ref UInt32 error_code)
        {
            MIB_UDPSTATS m=new MIB_UDPSTATS();
            error_code=GetUdpStatistics(ref m);
            if ((error_code!=0) && b_verbose)// error
            {
                string str_msg=Tools.API.API_error.GetAPIErrorMessageDescription(error_code);
                System.Windows.Forms.MessageBox.Show( str_msg,"Error",
    ...
    public static MIB_TCPSTATS GetTcpStatistics()
    {
        UInt32 error_code=0;
        return iphelper.GetTcpStatistics(true,ref error_code);
    }
